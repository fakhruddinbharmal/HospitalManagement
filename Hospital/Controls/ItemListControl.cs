...
{
    public partial class ItemListControl : Hospital.Controls.ObjectListControl
    {
        ...
        private void LoadListData(Item selected)
        {
            int count = 0;
            this.LoadEntityList<Item>(this.dgvData, this.clmItem.Index, new ItemCollection(tstSearch.Text), selected,
                delegate(DataGridViewRow row, Item obj)
                {
                    count++;
        ...
        {
            return GetSelectedEntity(this.dgvData) as Item;
        }
        ...
        private Item GetSelectedEntityObject()
        {
            return GetSelectedEntity(this.dgvData) as Item;
        }
