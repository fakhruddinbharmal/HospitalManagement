...
namespace Hospital.Object
{
    internal static partial class AppDAL
    {
        ...
        internal static bool MedicineInsert(Guid Medicineguid, string MedicineName, string Description, Guid createdByUser, out DateTime createdOn)
        {
            bool r = false;
            createdOn = DateTime.MinValue;
            using (SqlCommand cmd = AppDatabase.GetStoreProcCommand(Medicine_Insert))
            {
                MedicineParameters(cmd, Medicineguid, MedicineName, Description, createdByUser);
